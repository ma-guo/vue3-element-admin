...
const handleQuery = async () => {
  // 重置父组件
  loading.value = true;
  const rsp = await getMenusList(queryParams);
  loading.value = false;
  if (rsp.result == 0) {
    menuList.value = rsp.data.items;
  }
}
 ...
 * @param menuId 菜单ID
 */
const openDialog = async (parentId: number, menuId?: number) => {
  const rsp = await getMenusOptions({});
  if (rsp.result != 0) {
    return;
  }
  menuOptions.value = [
    { value: 0, label: "顶级菜单", children: rsp.data.items },
  ];
  dialog.visible = true;
  if (menuId) {
    dialog.title = "编辑菜单";
    const rsp2 = await getMenusForm({ id: menuId });
    if (rsp2.result == 0) {
      Object.assign(formData, rsp2.data);
      menuCacheData.type = rsp2.data.type;
      menuCacheData.path = rsp2.data.path ?? "";
    } else {
      ElMessage.error(rsp2.message);
    }
  } else {
    dialog.title = "新增菜单";
...
function submitForm() {
  menuFormRef.value.validate(async (isValid: boolean) => {
    if (isValid) {
      ...
      if (menuId) {
        const rsp = await setMenusUpdate(formData);
        if (rsp.result == 0) {
        ...
        const rsp = await setMenusAdd(formData);
        if (rsp.result == 0) {
...
function handleDelete(menuId: number) {
  ...
  ElMessageBox.confirm("确认删除已选中的数据项?", "警告", {
    ...
    const rsp = await setMenusDelete({ ids: [menuId].join(",") });
    if (rsp.result == 0) {
