...
const handleQuery = async () => {
  loading.value = true;
  const rsp = await getRolesPage(queryParams);
  loading.value = false;
  if (rsp.result != 0) {
    return;
  }
...
const openDialog = async (roleId?: number) => {
  ...
  if (roleId) {
    dialog.title = "修改角色";
    const rsp = await getRolesForm({ id: roleId });
    if (rsp.result == 0) {
...
function handleSubmit() {
  roleFormRef.value.validate(async (valid: any) => {
    if (valid) {
      ...
      if (roleId) {
        const rsp = await setRolesUpdate(formData);
        loading.value = false
        if (rsp.result == 0) {
        ...
        const rsp = await setRolesAdd(formData);
        loading.value = false
        if (rsp.result == 0) {
